...
defmodule RadarrRenameTest do
  @moduledoc """
  Standalone integration test for Radarr rename functionality.

  Usage:
    elixir test_radarr_rename.exs

  Or with explicit config:
    RADARR_URL=http://localhost:7878 RADARR_API_KEY=your_key elixir test_radarr_rename.exs

  This test:
    1. Picks a file from Radarr
    2. Renames it on disk (simulating post-reencode state)
    3. Triggers a refresh so Radarr detects the change
  ...
  def run do
    ...
    IO.puts("\n" <> String.duplicate("=", 60))
    IO.puts("RADARR RENAME INTEGRATION TEST")
    IO.puts(String.duplicate("=", 60))
    IO.puts("URL: #{config.url}")

    with :ok <- test_connection(config),
         {:ok, movie_id, file_id, original_path} <- pick_test_file(config),
         {:ok, mangled_path} <- mangle_filename(original_path),
         {:ok, command_id} <- test_refresh(config, movie_id),
         :ok <- wait_for_command(config, command_id, "RefreshMovie"),
         {:ok, renameable} <- test_renameable_files(config, movie_id),
         {:ok, _} <- test_rename(config, movie_id, renameable, original_path) do
      IO.puts("\n✅ All tests passed!")
      :ok
    else
      {:skip, reason} ->
  ...
  defp get_config do
    ...
    case {System.get_env("RADARR_URL"), System.get_env("RADARR_API_KEY")} do
  ...
  defp read_config_from_db do
    IO.puts("Reading Radarr config from #{@db_path}...")

    {:ok, conn} = Exqlite.Sqlite3.open(@db_path)

    {:ok, stmt} =
      Exqlite.Sqlite3.prepare(conn, "SELECT url, api_key FROM configs WHERE service_type = 'radarr'")
    ...
    case Exqlite.Sqlite3.step(conn, stmt) do
      ...
      :done ->
        Exqlite.Sqlite3.release(conn, stmt)
        Exqlite.Sqlite3.close(conn)
        raise "No Radarr config found in database. Set RADARR_URL and RADARR_API_KEY environment variables."
    end
  end
  ...
  defp test_connection(config) do
    ...
    case request(config, url: "/api/v3/system/status", method: :get) do
      {:ok, %{status: 200, body: body}} ->
        IO.puts("   ✅ Connected to Radarr v#{body["version"]}")
        :ok
  ...
  defp pick_test_file(config) do
    IO.puts("\n🎬 Fetching movies with files...")

    with {:ok, %{body: movies}} <- request(config, url: "/api/v3/movie", method: :get),
         movie when not is_nil(movie) <- find_movie_with_file(movies),
         {:ok, %{body: files}} <- request(config, url: "/api/v3/moviefile?movieId=#{movie["id"]}", method: :get),
         file when not is_nil(file) <- List.first(files) do

      IO.puts("   Selected movie: #{movie["title"]} (ID: #{movie["id"]})")
      IO.puts("   File ID: #{file["id"]}")
      IO.puts("   Path: #{file["path"]}")

      {:ok, movie["id"], file["id"], file["path"]}
    else
      nil ->
        IO.puts("   ❌ No movies with files found")
        {:error, :no_files}
  ...
  defp find_movie_with_file(movies) do
    movies
    |> Enum.filter(&(&1["hasFile"] == true))
    |> List.first()
  end
  ...
  defp test_refresh(config, movie_id) do
    IO.puts("\n🔄 Refreshing movie #{movie_id}...")
  ...
  defp test_renameable_files(config, movie_id) do
    IO.puts("\n📋 Checking renameable files for movie #{movie_id}...")

    case request(config, url: "/api/v3/rename?movieId=#{movie_id}", method: :get) do
      {:ok, %{status: 200, body: files}} when is_list(files) ->
        if Enum.empty?(files) do
          ...
          Enum.each(files, fn file ->
            IO.puts("")
            IO.puts("   Movie File ID: #{file["movieFileId"]}")
  ...
  defp test_rename(_config, _movie_id, [], _original_path) do
    IO.puts("\n⚠️  No files to rename - Radarr didn't detect the change")
    IO.puts("   This is the bug! The refresh should have detected the renamed file.")
    {:error, :no_renameable_files_detected}
  end

  defp test_rename(config, movie_id, renameable_files, original_path) do
    IO.puts("\n🔧 Executing rename for movie #{movie_id}...")

    file_ids = Enum.map(renameable_files, & &1["movieFileId"])
    ...
    case request(config, url: "/api/v3/command", method: :post, json: payload) do
      {:ok, %{status: status, body: body}} when status in [200, 201] ->
        ...
        with :ok <- wait_for_command(config, command_id, "RenameFiles") do
          verify_rename(config, movie_id, renameable_files, original_path)
        end
  ...
  defp verify_rename(config, movie_id, original_renameable, original_path) do
    ...
    if file_restored do
      ...
      # Check what path Radarr thinks the file is at now
      case request(config, url: "/api/v3/rename?movieId=#{movie_id}", method: :get) do
    ...
    case request(config, url: "/api/v3/rename?movieId=#{movie_id}", method: :get) do
      {:ok, %{body: new_renameable}} when is_list(new_renameable) ->
        original_ids = MapSet.new(original_renameable, & &1["movieFileId"])
        new_ids = MapSet.new(new_renameable, & &1["movieFileId"])
