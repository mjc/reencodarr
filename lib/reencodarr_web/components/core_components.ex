defmodule ReencodarrWeb.CoreComponents do
  ...
  def flash(assigns) do
    ...
    <div
      ...
      class={[
        ...
        @kind == :info && "bg-emerald-900/80 text-emerald-200 ring-emerald-500 fill-emerald-300",
        @kind == :error && "bg-rose-900/80 text-rose-200 shadow-md ring-rose-500 fill-rose-300"
      ]}
      {@rest}
    >
      ...
      <button type="button" class="group absolute top-1 right-1 p-2" aria-label={gettext("close")}>
        <span aria-hidden="true" class="text-lg leading-none opacity-40 group-hover:opacity-70">
          &times;
        </span>
      </button>
    </div>
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(ReencodarrWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(ReencodarrWeb.Gettext, "errors", msg, opts)
    end
  end
