defmodule Reencodarr.AbAv1.CrfSearcher do
  @moduledoc """
  Named port-holder for ab-av1 crf-search operations.

  Mirrors `AbAv1.Encoder` but for the CRF search pipeline. Owns the OS port
  for a single CRF search job and is designed to survive restarts of
  `AbAv1.CrfSearch`. Buffers output and forwards to a subscriber.

  Started on-demand under `Reencodarr.PortSupervisor` with `:temporary`
  restart policy.
  """
  ...
  @doc "Start the port holder under PortSupervisor. Called by AbAv1.CrfSearch."
  ...
  @doc "Check whether a CRF search is currently in flight."
  ...
  @doc "Return the CRF search metadata stored at start time."
  ...
  Used by `AbAv1.CrfSearch.reset_if_stuck/0`.
  """
  @spec kill() :: :ok
  def kill do
    case GenServer.whereis(__MODULE__) do
  ...
  def init({args, metadata}) do
    ...
    case Helper.open_port(args) do
      {:ok, port} ->
        ...
        Logger.info("CrfSearcher: port opened (OS PID: #{os_pid})")
      ...
      {:error, reason} ->
        Logger.error("CrfSearcher: failed to open port: #{inspect(reason)}")
        {:stop, reason}
    end
  end
  ...
  def terminate(:shutdown, state) do
    Logger.info("CrfSearcher: shutdown, killing OS process #{state.os_pid}")
    Helper.kill_os_process(state.os_pid)
    Helper.close_port(state.port)
    :ok
  end
